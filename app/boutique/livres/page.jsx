...
export default function LivresPage() {
  ...
  function handleBuy(product, email, setLoading) {
    ...
    if (product === 'epub') {
      ...
      fetch('/api/checkout-livre', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product, email }) })
        .then(r => r.json())
        .then(data => { if (data.url) window.location.href = data.url; else { setError(data.error || 'Une erreur est survenue.'); setLoading(false) } })
  ...
  async function handleReviewSubmit(e) {
    ...
    try {
      ...
      if (res.ok) {
        setReviewState('done'); setReviewName(''); setReviewRating(0); setReviewComment('')
        fetch('/api/reviews').then(r => r.json()).then(setReviews).catch(() => {})
    ...
    <main className="min-h-screen page-glow">
      ...
      <header className="border-b border-border px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center gap-2 text-sm">
          ...
          <span className="text-border mx-1">›</span>
          <span className="text-cream">Livres</span>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        {/* Titre + sous-titre */}
        <div className="mb-10">
          <p className="text-xs font-medium text-gold/60 uppercase tracking-widest mb-2">📖 Livres</p>
          <h1 className="font-[family-name:var(--font-playfair)] text-3xl font-bold text-cream">L'urgence des temps</h1>
          <p className="text-muted text-sm mt-1">Nicolas Salafranque · Éditions Le Scribe · 2026 · 211 pages · ISBN 979-1098694301</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-10 items-start mb-10">
          <div className="flex-shrink-0 mx-auto md:mx-0">
            <img
              ...
              className="w-44 rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.7)]"
            />
          </div>
          <div className="flex-1">
            ...
            <button
              ...
              className="text-sm font-semibold text-gold border border-gold/40 px-5 py-2 rounded-xl hover:bg-gold/10 transition"
            >
        ...
        {excerptOpen && (
          ...
          >
            <div style={{ background: '#faf7f2', borderRadius: '16px', padding: '40px 36px', maxWidth: '680px', width: '100%', position: 'relative', boxShadow: '0 30px 80px rgba(0,0,0,.6)', marginTop: '20px', marginBottom: '20px' }}>
              <button onClick={() => setExcerptOpen(false)} style={{ position: 'absolute', top: '16px', right: '20px', background: 'transparent', border: 'none', fontSize: '22px', color: '#9a8060', cursor: 'pointer', lineHeight: 1 }}>×</button>
              <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '11px', fontWeight: 600, color: '#9a8060', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '24px' }}>Introduction</p>
              {buildBlocks(INTRO_RAW)}
              <hr style={{ border: 'none', borderTop: '1px solid #d8d0c0', margin: '40px 0 32px' }} />
              <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '11px', fontWeight: 600, color: '#9a8060', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '8px' }}>Chapitre 1</p>
              <p style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '22px', fontWeight: 700, color: '#1c1917', marginBottom: '24px', lineHeight: 1.3 }}>Pourquoi tant de chrétiens se sont trompés</p>
              {buildBlocks(CH1_RAW)}
              <p style={{ textAlign: 'center', fontFamily: 'Georgia, serif', fontSize: '14px', color: '#9a8060', marginTop: '32px', fontStyle: 'italic' }}>— Fin de l'extrait —</p>
              <div style={{ textAlign: 'center', marginTop: '24px' }}>
                <button onClick={() => setExcerptOpen(false)} style={{ background: '#b08e63', color: '#fff', fontWeight: 700, fontSize: '14px', padding: '12px 24px', borderRadius: '10px', border: 'none', cursor: 'pointer' }}>
                  Commander le livre →
                </button>
              </div>
            </div>
        ...
        {/* Erreur */}
        {error && (
          <div className="mb-8 bg-err/10 border border-err/20 text-err text-sm rounded-xl px-4 py-3 text-center">{error}</div>
        )}
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-4">
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 flex flex-col">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="font-[family-name:var(--font-playfair)] text-xl font-bold text-cream mt-1">EPUB</h2>
                <p className="text-muted text-xs mt-1">Téléchargement immédiat · Compatible toutes liseuses</p>
              </div>
              <span className="text-2xl font-bold text-cream flex-shrink-0">9€</span>
            </div>
            ...
            <button onClick={() => handleBuy('epub', emailEpub, setLoadingEpub)} disabled={loadingEpub}
          ...
          <div className="bg-surface border border-gold/20 rounded-2xl p-6 flex flex-col relative overflow-hidden">
            <div className="absolute top-4 right-4">
              <span className="text-xs font-medium bg-gold/10 text-gold border border-gold/30 px-2 py-0.5 rounded-full">Disponible</span>
            </div>
            <div className="flex items-start justify-between mb-4">
              <div>
                <span className="text-xs font-medium text-gold/60 uppercase tracking-widest">Papier</span>
                <h2 className="font-[family-name:var(--font-playfair)] text-xl font-bold text-cream mt-1">Livre physique</h2>
                <p className="text-muted text-xs mt-1">Éditions Le Scribe · 211 pages</p>
              </div>
              <span className="text-2xl font-bold text-cream flex-shrink-0 mt-5">18,99€</span>
            </div>
            <ul className="text-sm text-muted space-y-1.5 mb-6 flex-1">
            ...
            <input type="email" placeholder="ton@email.com" value={emailLivre} onChange={e => setEmailLivre(e.target.value)}
              className="w-full text-sm bg-surface2 border border-border rounded-lg px-4 py-2.5 text-cream placeholder:text-muted2 focus:outline-none focus:border-gold/50 transition mb-3" />
            <button onClick={() => handleBuy('livre', emailLivre, setLoadingLivre)} disabled={loadingLivre}
              className="w-full bg-gold text-bg font-semibold text-sm py-3 rounded-xl hover:bg-gold2 transition disabled:opacity-60">
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 flex flex-col">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="font-[family-name:var(--font-playfair)] text-xl font-bold text-cream mt-1">3 exemplaires</h2>
                <p className="text-muted text-xs mt-1">16 € / ex. · Économisez 8,97 € vs prix unitaire</p>
              </div>
              <span className="text-2xl font-bold text-cream flex-shrink-0">48€</span>
            </div>
            <ul className="text-sm text-muted space-y-1.5 mb-6 flex-1">
            ...
            <input type="email" placeholder="ton@email.com" value={emailPack3} onChange={e => setEmailPack3(e.target.value)}
              className="w-full text-sm bg-surface2 border border-border rounded-lg px-4 py-2.5 text-cream placeholder:text-muted2 focus:outline-none focus:border-gold/50 transition mb-3" />
            <button onClick={() => handleBuy('pack3', emailPack3, setLoadingPack3)} disabled={loadingPack3}
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 flex flex-col">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="font-[family-name:var(--font-playfair)] text-xl font-bold text-cream mt-1">10 exemplaires</h2>
                <p className="text-muted text-xs mt-1">14 € / ex. · Remise spéciale communautés</p>
              </div>
              <span className="text-2xl font-bold text-cream flex-shrink-0">140€</span>
            </div>
            <ul className="text-sm text-muted space-y-1.5 mb-6 flex-1">
            ...
            <input type="email" placeholder="ton@email.com" value={emailPack10} onChange={e => setEmailPack10(e.target.value)}
              className="w-full text-sm bg-surface2 border border-border rounded-lg px-4 py-2.5 text-cream placeholder:text-muted2 focus:outline-none focus:border-gold/50 transition mb-3" />
            <button onClick={() => handleBuy('pack10', emailPack10, setLoadingPack10)} disabled={loadingPack10}
              className="w-full bg-surface2 border border-border text-cream font-semibold text-sm py-3 rounded-xl hover:border-gold/40 transition disabled:opacity-60">
        ...
        <p className="text-center text-xs text-muted2 mt-6 mb-16">
          Paiement sécurisé par Stripe · Carte bancaire, Apple Pay, Google Pay
        </p>

        {/* ── Témoignages ─────────────────────────────────────────── */}
        <div className="pt-10 border-t border-border">
          <h3 className="font-[family-name:var(--font-playfair)] text-2xl font-bold text-cream mb-8">Ce que disent les lecteurs</h3>
        ...
        <div className="mt-16 pt-10 border-t border-border">
          <div className="flex items-end gap-4 mb-8">
            <h3 className="font-[family-name:var(--font-playfair)] text-2xl font-bold text-cream">Avis des lecteurs</h3>
            {reviews.length > 0 && (
              <div className="flex items-center gap-2 mb-0.5">
                <Stars value={Math.round(avgRating)} size="text-base" />
                <span className="text-sm text-muted">{avgRating.toFixed(1)} · {reviews.length} avis</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-surface border border-border rounded-2xl p-6">
            <h4 className="font-[family-name:var(--font-playfair)] text-lg font-bold text-cream mb-5">Laisser un avis</h4>
            {reviewState === 'done' ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => setReviewState('idle')} className="mt-4 text-xs text-gold hover:text-gold2 transition">Laisser un autre avis</button>
              </div>
            ) : (
              <form onSubmit={handleReviewSubmit} className="flex flex-col gap-4">
                <div>
                  <label className="text-xs text-muted block mb-1.5">Votre note *</label>
                  <Stars value={reviewRating} hover={reviewHover} size="text-3xl" interactive onHover={setReviewHover} onClick={setReviewRating} />
                </div>
                <div>
                  <label className="text-xs text-muted block mb-1.5">Votre prénom *</label>
                  <input type="text" required placeholder="Jean-Pierre" value={reviewName} onChange={e => setReviewName(e.target.value)} maxLength={100}
                ...
                <div>
                  ...
                  <textarea required placeholder="Partagez votre ressenti sur ce livre…" value={reviewComment} onChange={e => setReviewComment(e.target.value)} maxLength={1000} rows={4}
                ...
                {reviewState === 'error' && <p className="text-xs text-err">Une erreur est survenue. Réessayez.</p>}
                <button type="submit" disabled={reviewState === 'loading' || !reviewRating}
