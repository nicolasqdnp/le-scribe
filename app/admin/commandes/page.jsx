...
export default function CommandesAdmin() {
  ...
  const [sortP, setSortP] = useState({ col: 'created_at', dir: 'desc' })
  const [sortN, setSortN] = useState({ col: 'created_at', dir: 'desc' })
  ...
  function sortRows(arr, sort) {
    return [...arr].sort((a, b) => {
      ...
      else if (sort.col === 'address') { va = formatAddr(a.shipping_address).toLowerCase(); vb = formatAddr(b.shipping_address).toLowerCase() }
              ...
              <table style={{ minWidth: 900, borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                    ...
                    <ThHead label="Adresse" col="address" sort={sortP} onSort={col => handleSort(setSortP, col)} C={C} />
